...
// writePeerEnvFile writes an empty env file for a peer and returns its path.
func (n *TestNode) writePeerEnvFile() string {
	t := n.env.t
	t.Helper()
...
func NewTestNode(t *testing.T, env *TestEnv, opts ...TestNodeOpt) *TestNode {
	...
	if env.windowsService {
		// The role decides the socket and state paths, since a LocalSystem service
		// ignores --socket/--statedir and uses the defaults.
...
func (d *Daemon) MustCleanShutdown(t testing.TB) {
	...
	if runtime.GOOS == "windows" {
		if err := gracefulStop(d.Process); err != nil {
			t.Logf("graceful stop: %v; killing", err)
			d.Process.Kill()
			d.Process.Wait()
			return
		}
		...
		case <-time.After(30 * time.Second):
			t.Errorf("tailscaled did not exit within 30s of shutdown signal; killing")
			d.Process.Kill()
			<-done
		}
		return
...
func (n *TestNode) StartDaemonAsIPNGOOS(ipnGOOS string) *Daemon {
	...
	if runtime.GOOS != "windows" {
		...
		// Point the peer at its own env file so it doesn't read the service's.
		cmd.Env = append(cmd.Env, "TS_DEBUG_ENV_FILE="+n.writePeerEnvFile())
...
// windowsUpArgs adds --unattended to a Windows "tailscale up" so its state persists; see #20751.
func windowsUpArgs(args ...string) []string {
	out := slices.Clone(args)
	if runtime.GOOS == "windows" && len(out) > 0 && out[0] == "up" && !slices.Contains(out, "--unattended") {
...
func (n *TestNode) MustUp(extraArgs ...string) {
	...
	cmd := n.Tailscale(windowsUpArgs(args...)...)
