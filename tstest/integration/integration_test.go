...
func TestStateSavedOnStart(t *testing.T) {
	...
	upArgs := windowsUpArgs("up", "--login-server="+n1.env.ControlURL(), "--hostname=foo")
	...
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for i, step := range tt.steps {
				...
				cmdArgs := windowsUpArgs(append(slices.Clone(step.args), "--login-server="+env.ControlURL())...)
...
func TestOneNodeUpInterruptedAuth(t *testing.T) {
	...
	cmdArgs := windowsUpArgs("up", "--login-server="+env.ControlURL())
...
func TestOneNodeUpInterruptedDeviceApproval(t *testing.T) {
	...
	cmd1Args := windowsUpArgs("up", "--login-server="+env.ControlURL())
	...
	// This stays a bare "up" (no windowsUpArgs) to exercise the simple-up path.
	cmd2Args := []string{"up"}
...
func TestConfigFileAuthKey(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("--config is unsupported by the Windows service; see #20750")
	}
	t.Parallel()
...
func TestEncryptStateMigration(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("--encrypt-state is unsupported by the Windows service; see #20750")
	}
