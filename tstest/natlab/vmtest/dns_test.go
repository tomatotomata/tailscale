...
// resolver. One row of the checks table below per kind:
//
...
//     tailscaled's own resolver (100.100.100.100, "quad-100") from an extra
//     record control sent.
...
//
// The mixed routes below leave no single resolver set to hand the OS, so
// tailscaled forwards case 1 via quad-100 rather than letting the OS do it (see
// compileConfig in net/dns/manager.go). [TestSplitDNSOSForwarded] covers the
// other arrangement.
//
...
func testSplitDNS(t *testing.T, dnsMode vmtest.DNSMode, wantBackend string) {
	...
	// Case 1 (forwarded). vnet's second DNS server is the *only* thing serving
	// fwdName, so an answer means the query was forwarded there.
	const (
	...
	// in it. localIP is never connected to, only resolved; it's outside the
	// 100.64.x.y block testcontrol assigns nodes, so an answer can only have come
	// from the extra record.
	const (
	...
	// Case 3's lookup and expected answer. Checking the name against what control
	// should have assigned, rather than trusting whatever it reports, means a
	// MagicDNS naming change fails here instead of quietly resolving some other
	// name and passing.
	...
	checks := []struct {
	...
	}{
		{
			name: fwdName,
			want: fwdIP,
			what: "1. forwarded: routed domain went to its designated resolver",
		},
		{
			name: localName,
			want: localIP,
			what: "2. local: empty-resolver route answered by quad-100",
		},
		// Same answer, but looked up as a bare name to check the search domain.
		{
			name: "host",
			want: localIP,
			what: "2. local: bare name completed by the " + localDomain + " search domain",
		},
		// quad-100 answers tailnet names from the netmap.
		{
			name: peerName,
			want: peerIP,
			what: "3. magicdns: peer's tailnet name resolved to its tailnet IP",
		},
		...
		// (here vnet's default one, standing in for the internet).
		{
			...
			want: "5.0.0.100",
			what: "4. default: unrouted name still resolved normally",
		},
	}
